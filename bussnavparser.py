...
class BusinessNavigatorParser:
    ...
    def _get_max_page(self):
        ...
        page = req.get(self.ROOT + self.COMPANIES)
        html = page.text
        soup = BeautifulSoup(html, 'lxml')
        urls = soup.find("div", class_="bx_pagination_page").find_all("a",
                                                                      text=re.compile(self._re_search_detail_template))
    ...
    def get_data_by_pagin_page(self, num):
        ...
        data = []
        page_url = self.ROOT + self.COMPANIES + page_url_templ.format(num)
    ...
    def get_data_by_pagin_range(self, start, end):
        ...
        for i in range(start, end + 1):
            page_url = self.ROOT + self.COMPANIES + page_url_templ.format(i)
            urls = self._get_table_urls(page_url)
            for url in urls:
                temp = self._get_company_data(self.ROOT + url)
                data.append(temp)
